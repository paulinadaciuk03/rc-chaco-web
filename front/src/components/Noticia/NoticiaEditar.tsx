...
export default function NoticiaEditar() {
    ...
    if (id) {
      getNoticiaById(parseInt(id))
        ...
        .catch((error) => {
          ...
          toast.error("Error al cargar la noticia para editar.");
        });
    }
  }, [id, reset]);
  ...
  const onSubmit = async (data: FormInputs) => {
    try {
      ...
      await updateNoticia(parseInt(id), payload);
      toast.success("Noticia editada con éxito");
      navigate("/novedades");
    } catch (error) {
      console.error("Error al editar la noticia:", error);
      toast.error("Ocurrió un error al editar la noticia");
    }
  };
 ...
 <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
      <div className="space-y-2 mb-8">
        <h1 className="text-2xl sm:text-3xl font-bold text-sky-900">Editar noticia</h1>
